...
nosey.on('project_found', function(path, name) {
  var project = nosey.projects[path] =
    { path: path
    , name: name
    , status: 'new'
...
nosey.findProject = function(name) {
  var path
  for (path in nosey.projects) {
    if (!nosey.projects.hasOwnProperty(path)) continue
    if (nosey.projects[path].name == name) return nosey.projects[path]
  }
  return null
}
